...
M_total = 100   # output dimension
k_total = 10     # approx # active bits

#embed = SpatialBumpMapUnique(M_total, k_total, width, height)
embed = SpatialBumpMapUnique(M_total, k_total, crop_size, crop_size)
...
# ╔═╡ e3f3ce27-3c98-4a65-848d-469d6f2e7dbc
embed

# ╔═╡ c36553ba-9664-44ec-9642-08cd43395df1
X = [10  11  20  30;
     10  11  21  31]     # size = (2, 4)
...
# ╔═╡ 52444bbb-9f3a-4668-b9d1-89894000a98f
############################################################
# 2. Build all coordinates of a K×K image and compute collisions
############################################################

"""
    collision_stats(K, M_total, k_total; rng=Random.GLOBAL_RNG)

- Constructs all coordinates (x,y) with x,y in 0:(K-1).
- Embeds them with SpatialBumpMapUnique(M_total, k_total, K, K).
- Counts how many coordinates share the same embedding.

Returns a NamedTuple with:
- total_points::Int            = K^2
- collision_points::Int        = number of coords in groups of size ≥ 2
- collision_groups::Int        = number of distinct embeddings with freq ≥ 2
- max_group_size::Int          = largest collision group size
"""
begin
function collision_stats(K::Int, M_total::Int, k_total::Int;
                         rng=Random.GLOBAL_RNG)

    # Build the embedder
    embed = SpatialBumpMapUnique(M_total, k_total, K, K; rng=rng)

    # Build all coordinates: x,y ∈ 0:(K-1)
    N = K * K
    ...
    # Compute embeddings: Bool matrix of size M_total×N
    Z = embed(X)

    # Map each column to a BitVector key and count frequencies
    counts = Dict{BitVector, Int}()

    for j in 1:N
        key = BitVector(Z[:, j])  # copy of column j as BitVector
...
# ╔═╡ 57f83fb9-9887-4f08-acb1-19837a06ada2
begin
	
############################################################
# 3. Example: run it for K=40, ~10% sparsity
############################################################

# Example parameters
#K       = 40
#M_total = 1004      # e.g. 1000 bump dims + 4 color bits
#k_total = Int(round(0.1 * M_total))  # about 10% ones per code

stats = collision_stats(crop_size, M_total, k_total)

println("crop_size = $crop_size, M_total = $M_total, k_total ≈ $k_total")
println("Total points:      ", stats.total_points)
println("Collision points:  ", stats.collision_points)
println("Collision groups:  ", stats.collision_groups)
...
manifest_format = "2.0"
project_hash = "3a3cdafffe7ab6176e0db231b7d4d1006f1f55d2"
