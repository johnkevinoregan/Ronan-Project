...
# ╔═╡ 5c8a68f3-dc00-42da-b38e-bc194c17282b


"""
    hash_k_sparse_exact(x, y; M, k=4, seed=0x12345678)

Map integer coordinates (x,y) to an M-dimensional K-hot sparse binary vector,
with exactly k distinct 1-entries.

- M: dimensionality of the embedding (must be ≥ k)
- k: number of active bits
- seed: unsigned integer for deterministic hashing (UInt32 or UInt64 ok)
"""
function hash_k_sparse_exact(x::Integer, y::Integer; M::Integer,
                             k::Integer = 4, seed::Unsigned = 0x12345678)
    @assert M > 0 "M must be positive"
    @assert k > 0 "k must be positive"
    @assert M ≥ k "M must be at least k to get k distinct bits"

    idxset = Set{Int}()
    i = 1

    # Ensure the seed is a UInt for Base.hash
    hseed = UInt(seed)

    while length(idxset) < k
        h = hash((x, y, i), hseed)         # second arg is now UInt
        j = Int(rem(h, UInt(M)) + 1)       # map to 1..M
        push!(idxset, j)
        i += 1
    end
    ...
    # EITHER let Julia infer types:
    # return SparseVector(M, idxs, vals)

    # OR explicitly specify value and index types:
