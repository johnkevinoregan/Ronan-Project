...
# ╔═╡ a4aec70a-fcca-42e5-8726-5042910e27d2
X_train

# ╔═╡ ba9b4c08-3ff7-49e6-b785-7f0ef8387f04
img


# ╔═╡ 95a8b378-2595-4b54-b6e6-32f65bd28abf
X_train

# ╔═╡ b9d23258-ca42-11f0-81f0-03c4b59bac18
begin
...
# ╔═╡ 8d973cfa-a0f0-4bbc-bf92-4b1d9046cb32
Y_train

# ╔═╡ de5d4fa2-cd39-43a1-96ca-d5773e6ff137
begin
	struct FourierFeature
	    B::Matrix{Float32} # The Gaussian matrix
	end
	
	# Constructor
	function FourierFeature(input_dim::Int, mapping_size::Int, scale::Float32)
	    ...
	    # Initialise B from Gaussian distribution * scale
	    B = randn(Float32, b_rows, input_dim) .* scale
	    return FourierFeature(B)
	end
	
	# The Forward Pass
	function (f::FourierFeature)(x)
	    # Projection
	    projection = 2f0 * π .* (f.B * x)
	    
	    # Fourier Features
	    return Float32.(vcat(cos.(projection), sin.(projection)))
	end
end
...
begin
	...
	# Define the Model
	model = Chain(
	    FourierFeature(2, mapping_size, scale), # Inputs: 2 -> 256
...
# ╔═╡ f361d735-f1f5-40c7-8c90-1ae07c09b0e5
recon = reshape(model(X_train), (3, 25, 25))

# ╔═╡ 2b0d7e08-97eb-4f13-a6d4-4c6759ee0611
colorview(RGB, recon)
...
manifest_format = "2.0"
project_hash = "48416afd6c58fe0d91b412c011f85614c478efba"
