...
begin
	...
	# Constructor for size-based initialisation
	Layer(in::Int, out::Int, activation::Function=Flux.identity) =
	    Layer(randn(Float32, out, in), randn(Float32, out), activation)
...
begin
	...
	# Constructor for vararg layer input
	Network(layers::Vararg{Layer}) = Network(collect(layers))
...
begin
	...
	inputs = Float32.(reshape(collect(-3:0.1:3),(1,:))) # create training data
	targetOutput = sin.(inputs)
	
	twoLayerNeuralNet = Network(Layer(1,100,Flux.relu), Layer(100,1)) 
end

# ╔═╡ 964321c6-c9e2-47c2-8d05-e0b73e5f2d2f
begin
	# Generate a range of values from -10 to 10
	x_values = Float32.(-10:0.1:10)
	
	# Apply the ReLU function to these values
	y_values = Flux.relu.(x_values)
...
# ╔═╡ e75a7d3c-091b-4d06-a2bf-97c7cbbdecdf
function loss(m, X, y)
	return mean((m(X) .- y).^2)
end

# ╔═╡ 5bfe25ba-a63c-4479-9a35-cf3874243d44
begin
	...
	opt = Flux.Adam(1f-3)
    opt_state = Flux.setup(opt, twoLayerNeuralNet)
	nepoch = 1000

	netOutput = [] # store output for plotting
	lossCurve = Float32[] # store loss for plotting

    for epoch in 1:nepoch
        grads = Flux.gradient(twoLayerNeuralNet) do m
			loss(m, inputs, targetOutput)
        end
        Flux.Optimise.update!(opt_state, twoLayerNeuralNet, grads[1])
		
		loss_val = loss(twoLayerNeuralNet, inputs, targetOutput)
		push!(netOutput, twoLayerNeuralNet(inputs))
        push!(lossCurve, loss_val)
...
@bind plotIndex PlutoUI.Slider(1:10:1000, default=100, show_value=true)

# ╔═╡ 139a0319-f5dc-423d-a236-55b8619940e5
let
	xvals = vec(inputs)             # flatten
	ytrue = vec(targetOutput)       # flatten
	ypred = vec(netOutput[plotIndex])
	
	fig = Figure()
	ax = Axis(fig[1, 1], title="Learning f(x) = sin(x)")
	
	lines!(ax, xvals, ypred, label="NN")
	scatter!(ax, xvals, ytrue, label="Target")
	
	axislegend(ax)
	fig
end

# ╔═╡ 5fa50b79-1631-4354-8ed6-ebbfe0f40fa5
let
	fig = Figure()
	ax = Axis(fig[1, 1], title="Training Loss")
	lines!(ax, 1:length(lossCurve), lossCurve)
...
manifest_format = "2.0"
project_hash = "5d2d1feec1a41ce2a1c2244e022b0ce197294c63"
